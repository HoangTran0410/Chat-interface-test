function getEmoji() {
    ...
    for (var tr of trs) {
        ...
        if (th && th[0]) {
            ...
            var code = td[2].innerHTML;
            var name = td[14].innerHTML;
...
function updateFrequentlyFromLocal() {
    var div = document.getElementById('frequently-used');
    var s = "";
...
function setupEmoji() {
    ...
    document.getElementsByClassName('composer-popover-input')[0].addEventListener('keyup', function() {
        ...
        for (var e of emojies) {
            if (e.title.indexOf(this.value) < 0)
                e.style.display = 'none';
            else e.style.display = 'inline-table';
        }
    })
}
